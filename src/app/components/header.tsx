...
export default function Header() {
    ...
    return (
        <header className={`w-full h-[90px] flex sticky top-0 z-50 ${isSticky ? 'backdrop-blur-2xl bg-[#27b17c11]' : 'bg-transparent'}`}>
            <div className="w-[1440px] h-full m-auto flex justify-between items-center ">
                ...
                <div className="w-[400px] h-[40px] border-[#646a6b71] border-[1px] flex items-center justify-center gap-16 rounded-[10px] backdrop-blur-md bg-gradient-to-l from-[#e3e3e30c]">
                    <Link href={"/launched"} className="text-[20px]">IDO Launchpad</Link>
                    <Link href={"/claims"} className="text-[20px]">Claims</Link>
                </div>
                <button className="w-[250px] h-[40px] rounded-[10px] bg-[#27B17D] flex items-center justify-center gap-3">
                    <Image
                        alt="wallet"
                        src={Wallet}
                    />
